...
def ZonalMean(data_array,
  ...
  if no_plev :
    ...
    if len(data_array.values.shape) != 3 and len(data_array.values.shape) != 4 :
      ...
      return 0
    zonal_mean = np.mean(tuple,axis = (0,2))
    dataarray_zm = xr.DataArray(data=zonal,dims=["lat"],coords = dict(lat=lat))
  if not no_plev:
    tuple = data_array.values
...
def ContourTracking2D(dataset,fn_out = "",var_name = "DAV",data_return = False, pers = 0,min_area = 500000):
  if fn_out=="" and data_return==False:
    string = "Specify the kind of output you want"
    print(string)
    return 0
  ...
  for t in tqdm(np.arange(0,len(dataset.time.values)-1)):
    if t > 0:
      tmp = np.amax(arr[t-1,:,:])
    ...
    for l in comp:
      ...
      #n = np.count_nonzero(boolarr)
      #filtering cluster dimension
      if area < min_area: #value chosen from litterature.
        arr[t,:,:] = xr.where(boolarr, 0,arr[t,:,:])
    ...
    if t > 0:
      ...
      for l1 in comp1:
        ...
        for l2 in comp2:
          ...
          diff = dataset.time.values[t]-dataset.time.values[t-1]
          diff = int(diff)
  ...
  #initialize dataset object for the new .nc
  pIB_tracked = xr.DataArray(0,coords=[times,lat,lon],dims = pIB_boolean.dims)
  pIB_tracked[:,:,:] = arr
  
  """
  Update DAV and DAV_freq (if present) after area and persistence filter are applied.
  """
  dataset[var_name] = pIB_tracked > 0
#  try: 
  dataset[var_name + "_freq"] = dataset[var_name].mean(dim="time")
#  except:
#    print("no " + var_name + "_freq variable found")

  #assign dataset to dataset which is now updated
  dataset = dataset.assign(pIB_tracked = pIB_tracked)
  dataset["pIB_tracked"].name = var_name + "_tracked"
